...
impl Solution {
    pub fn max_profit_single(prices: Vec<i32>) -> i32 {
        prices
            .iter()
...
impl Solution {
    ...
    {
        let s = x.to_string();
        let p = s.chars().rev().collect::<String>();
        s.eq(&p)
    }
}
...
mod tests {
    ...
    fn is_palindrome() {
        ...
        assert_eq!(Solution::is_palindrome("test"), false);
        assert_eq!(Solution::is_palindrome("abba"), true);
    }
    #[test]
    fn max_profit_single() {
        let z = vec![7, 1, 5, 3, 6, 4];
        assert_eq!(Solution::max_profit_single(z), 5);
    }
